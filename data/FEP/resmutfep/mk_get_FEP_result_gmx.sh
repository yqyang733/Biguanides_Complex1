get_fep_rt(){
    cd bonded
    for i in `seq 1 3`;do cd dup${i};mkdir temp;cd temp;for a in `seq 0 31`;do cp ../lambda${a}/dhdl.xvg dhdl_lambda${a}.xvg;done;gmx bar -f dhdl_lambda*.xvg -oi -o > fep.log;cd ../..;done
